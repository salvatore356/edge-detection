...
void applyKernel(Image &img, Pixel &gradient, int r, int c, Kernel &kernel) {
    for (int i = 0; i < kernel.height; ++i)
        for (int j = 0; j < kernel.width; ++j) {
            int rr = r + i - 1, cc = c + j - 1;
            gradient += (img.at(rr, cc) * kernel.at(i, j));
        }
}

float calcMagnitude(vector<Pixel> &gradients) {
    float magnitude = 0;
    
    for(auto &g: gradients)
        magnitude += g.avg() * g.avg();
    
    return std::sqrt(magnitude);
}

void applyKernels (Image &img, Image &filtered, int r, int c, vector<Kernel> &kernels) {
    vector<Pixel> gradients(kernels.size());

    for(int k = 0; k < kernels.size(); ++k)
        applyKernel(img, gradients[k], r, c, kernels[k]);
    
    filtered.at(r, c) = calcMagnitude(gradients);
}
